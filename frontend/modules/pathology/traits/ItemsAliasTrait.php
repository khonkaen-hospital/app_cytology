...
namespace frontend\modules\cytology\traits;
...
trait ItemsAliasTrait {
  ...
  {
    if(empty($this->{$field}) || in_array($this->{$field},['0000-00-00','0000-00-00 00:00:00']))
    {
        return $this->{$field};
    } else {
  ...
  *  '%a Days                                                =>  468 Days
  ...
  public function getCurrentAgeOnCN( $attribute1,$attribute2, $format = null )
  {
    if($format === null){
      $str   = ($y = (int) $this->dateDifference($this->getOldAttribute($attribute1), $this->getOldAttribute($attribute2), '%y')) > 0 ? $y.' ปี ' : null;
      $str   .= ($y = (int) $this->dateDifference($this->getOldAttribute($attribute1), $this->getOldAttribute($attribute2), '%m')) > 0 ? $y.' เดือน ' : null;
      $str   .= ($y = (int) $this->dateDifference($this->getOldAttribute($attribute1), $this->getOldAttribute($attribute2), '%d')) > 0 ? $y.' วัน': null;
      return $str;
    }
    return $this->dateDifference($this->getOldAttribute($attribute), date('Y-m-d'), $format);
  }
